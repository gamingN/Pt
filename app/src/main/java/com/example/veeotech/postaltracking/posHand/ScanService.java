...
public class ScanService extends Service {
	...
	private String str_result_back;
	...
	/**
	 * 扫描头扫描信息接收器
	 * postaltracking整個項目掃描後發送的自定義的廣播都在這裡,如需要可對應改動
	 */
...
//							sendBroadcast(intentBroadcast1);
							//设置当前客户可再次进行扫描
							str_result_back= (String) SPUtils.get(getApplicationContext(),IntentKeyUtils.SP_RESULT_BACK,"");
							if(str_result_back.equals("1")) {
								Intent broadcast = new Intent("android.intent.action.MEDICAL_BROADCAST");
								broadcast.putExtra(IntentKeyUtils.RESULT_BROADCAST, str.trim());
								Log.e("KIKI", "onReceive: 0" + str.trim());
								LocalBroadcastManager.getInstance(App.getInstance()).sendBroadcast(broadcast);
							}else if(str_result_back.equals("2")){
								Intent broadcast = new Intent("android.intent.action.MEDICAL2_BROADCAST");
								broadcast.putExtra(IntentKeyUtils.RESULT_BROADCAST, str.trim());
								Log.e("KIKI", "onReceive: 0" + str.trim());
								LocalBroadcastManager.getInstance(App.getInstance()).sendBroadcast(broadcast);
							}else if(str_result_back.equals("3")){
								Intent broadcast = new Intent("android.intent.action.MEDICAL3_BROADCAST");
								broadcast.putExtra(IntentKeyUtils.RESULT_BROADCAST, str.trim());
								Log.e("zwx", "onReceive: 0" + str.trim()+"发送广播3");
								LocalBroadcastManager.getInstance(App.getInstance()).sendBroadcast(broadcast);
							}else if(str_result_back.equals("4")){
								Intent broadcast = new Intent("android.intent.action.MEDICAL4_BROADCAST");
								broadcast.putExtra(IntentKeyUtils.RESULT_BROADCAST, str.trim());
								Log.e("zwx", "onReceive: 0" + str.trim()+"发送广播4");
								LocalBroadcastManager.getInstance(App.getInstance()).sendBroadcast(broadcast);
							}else if(str_result_back.equals("5")){
								Intent broadcast = new Intent("android.intent.action.MEDICAL5_BROADCAST");
								broadcast.putExtra(IntentKeyUtils.RESULT_BROADCAST, str.trim());
								Log.e("zwx", "onReceive: 0" + str.trim()+"发送广播5");
								LocalBroadcastManager.getInstance(App.getInstance()).sendBroadcast(broadcast);
							}else if(str_result_back.equals("6")){
								Intent broadcast = new Intent("android.intent.action.MEDICAL6_BROADCAST");
								broadcast.putExtra(IntentKeyUtils.RESULT_BROADCAST, str.trim());
								Log.e("zwx", "onReceive: 0" + str.trim()+"发送广播6");
								LocalBroadcastManager.getInstance(App.getInstance()).sendBroadcast(broadcast);
							}else if(str_result_back.equals("7")){
								Intent broadcast = new Intent("android.intent.action.MEDICAL7_BROADCAST");
								broadcast.putExtra(IntentKeyUtils.RESULT_BROADCAST, str.trim());
								Log.e("zwx", "onReceive: 0" + str.trim()+"发送广播7");
								LocalBroadcastManager.getInstance(App.getInstance()).sendBroadcast(broadcast);
							}
...
//							| AudioManager.FLAG_SHOW_UI);
			Toast.makeText(context, "F3點擊", Toast.LENGTH_SHORT).show();
		}
	}
